package lesson8;

public class Human implements Participant {

    public final int RUN_DISTANCE = 500;
    public final int JUMP_HEIGHT = 3;
    public boolean isDead = false;
    private final String name;

    public Human(String name) {
        this.name = name;
    }

    public String getName() {
        return name;
    }


    public void cross(Obstacle obstacle) {
        if (isDead) {
            return;
        }
        if (obstacle instanceof Track) {
            this.run(obstacle);
        } else if (obstacle instanceof Wall) {
            this.jump(obstacle);
        }
    }

    public void run(Obstacle track) {
        if (isDead) {
            return;
        } else if (track.getObstacleValue() <= RUN_DISTANCE)
            System.out.printf("Human %s runs %d metres\n", getName(), track.getObstacleValue());
        else if (track.getObstacleValue() > RUN_DISTANCE) {
            System.out.printf(" %s is dead. Dead  can not run or jump!\n", getName());
            isDead = true;
        }
    }


    public void jump(Obstacle wall) {
        if (isDead) {
            return;
        } else if (wall.getObstacleValue() <= JUMP_HEIGHT)
            System.out.printf("Human %s jumps %d metres\n", getName(), wall.getObstacleValue());
        else if (wall.getObstacleValue() > JUMP_HEIGHT) {
            System.out.printf(" %s is dead.  Dead can not run or jump!\n", getName());
            isDead = true;
        }
    }


}